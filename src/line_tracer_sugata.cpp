...
void updateState(Serial& ser, int total_white_pix, list<LINE>& line_list,int human_line);
int human_detect(Mat& src_img);
...
int main(int argc, char** argv){
    ...
    while(1){
      ...
      Mat src_img, resized_img, binalized_img;
        ...
        int human_line=human_detect(resized_img);

        // update a state
        updateState(ser,total_white_pix,center_line_list,human_line);
...
void updateState(Serial& ser, int total_white_pix, list<LINE>& line_list,int human_line){
    ...
    if(human_line != 9999){
      cout <<"Human detect at:"<<human_line<<endl;
    }
...
int human_detect(Mat& src_img){
  Mat hsv_video(Size(src_img.rows, src_img.cols), CV_8UC1, Scalar::all(255)); // HSVに変換した映像を保存する
  Mat dst_img(Size(src_img.rows, src_img.cols), CV_8UC1, Scalar::all(0)); // 認識結果を表示する

  cvtColor(src_img, hsv_video, CV_BGR2HSV);
  imshow("src",src_img);
  imshow("hsv",hsv_video);

  int pixel_array[5000];
  int data_count=0;
  ...
  for(int y = 0; y < hsv_video.rows; y++) {
    for (int x = 0; x < hsv_video.cols; x++) {
        ...
        // 居留地マップの検出
        if ((hue < 35 && hue > 20) && (sat < 255 && sat > 120)) {
            dst_img.at<uchar>(y, x) = 255;
            pixel_array[data_count]=x;
            data_count++;
        }else {
 ...
 sort_data(pixel_array, data_count);
 int mid;
 if(data_count %2 ==0){
   mid=pixel_array[data_count/2];
 }
 else{
     mid=(pixel_array[data_count/2-1] + pixel_array[data_count/2])/2;
   }
cout<<"中央値"<<mid<<endl;
cout<<"data_count"<<data_count<<endl;
line(src_img, Point(mid, 0), Point(mid, src_img.cols), Scalar(255,0,0), 10, CV_AA);
imshow("src_mid",src_img);
imshow("dst",dst_img);
